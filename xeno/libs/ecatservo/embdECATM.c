...
/*****************************************************************************/
int EcatInit(int cycleTime, int operationMode, uint64_t appStartTime){

	int i = 0;

	ec_slave_config_t *slaveConfRef = NULL;
	...
	for(i=0;i<LSMECAPION_SLAVENUM;i++){
		...
		/*6. DC setup*/
		ecrt_slave_config_dc(slaveConf, LSMECA_ACTIVATE_WORD, cycleTime, 700000, 0, 0); //wcs
		if (i == 0){
			slaveConfRef = slaveConf;
		}
	}
	printf("OK!\n");
	...
	ecrt_master_application_time(eMaster,appStartTime);
	ecrt_master_select_reference_clock(eMaster,slaveConfRef);
...
void EcatQuit(void){
	...
	printf("Clearing Process Domain Buffer...");
...
void lsmecaFaultReset(int iNode)
{
	...
	if (iNode == _ALLNODE){
		for(i=0;i<LSMECAPION_SLAVENUM;i++){
			EC_WRITE_U16(eDomain_pd + lsmecaCtrlWordOff[i],LSMECA_FAULT_RESET);

		}	
	}
	else{
	
		EC_WRITE_U16(eDomain_pd + lsmecaCtrlWordOff[iNode],LSMECA_FAULT_RESET);
	}
}
/*****************************************************************************/
